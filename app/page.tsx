...
import { ArrowRight, Award, CalendarDays, Clock3, Facebook, Instagram, MapPin, Menu, MessageCircle, Phone, Play, ShieldCheck, Sparkles, Star, Users, Youtube } from 'lucide-react'
...
const clinicPhotos = [
  ['/images/clinic-reception.png', 'Recepción luminosa de DentalBright'],
  ['/images/clinic-treatment.png', 'Consultorio dental con tecnología avanzada'],
  ['/images/clinic-lounge.png', 'Sala de espera cómoda y elegante'],
  ['/images/clinic-care.png', 'Especialista atendiendo a una paciente'],
  ['/images/clinic-consult.png', 'Consulta personalizada con nuestro equipo'],
  ['/images/clinic-sterilization.png', 'Área de esterilización clínica'],
]

export default function Home() {
  return <main>
    <header className="site-header"><a className="brand" href="#inicio" aria-label="DentalBright inicio"><ToothIcon/><strong>DentalBright</strong></a><nav aria-label="Navegación principal"><a className="active" href="#inicio">Inicio</a><a href="#servicios">Servicios</a><a href="#clinica">Sobre Nosotros</a><a href="#contacto">Contacto</a></nav><a className="button header-cta" href="#contacto"><CalendarDays size={18}/> Agenda tu cita</a><button className="menu-button" aria-label="Abrir menú"><Menu/></button></header>
    <section className="hero" id="inicio"><div className="hero-copy"><p className="eyebrow">ODONTOLOGÍA PREMIUM</p><h1>Sonrisas que<br/>transforman vidas</h1><p className="hero-lead">Cuidado dental avanzado con tecnología de vanguardia y un enfoque humano. Tu bienestar es nuestra prioridad.</p><div className="hero-actions"><a className="button" href="#contacto"><CalendarDays size={19}/> Agenda tu cita</a><a className="video-link" href="#clinica"><span><Play size={14} fill="currentColor"/></span> Conoce nuestra clínica</a></div><div className="social-proof"><div className="avatars"><i>AM</i><i>LR</i><i>CS</i><i>JM</i></div><div><div className="stars">{[1,2,3,4,5].map(n=><Star key={n}/>)}</div><small>Más de 1,200 pacientes satisfechos</small></div></div></div><div className="hero-visual"><Image src="/images/hero.png" alt="Paciente sonriente durante una revisión dental" fill priority sizes="(max-width: 1000px) 100vw, 54vw" /></div></section>
    ...
    <section className="clinic section" id="clinica"><div className="section-heading"><p className="eyebrow">NUESTRA CLÍNICA</p><h2>Diseñada para tu comodidad y confianza</h2><p>Espacios modernos, tecnología de vanguardia y un equipo humano<br/> comprometido con tu bienestar.</p></div><div className="gallery">{clinicPhotos.map(([src, alt]) => <figure className="clinic-photo" key={src}><Image src={src} alt={alt} fill sizes="(max-width: 640px) 100vw, 33vw" /></figure>)}</div></section>
    ...
    <footer><div className="footer-grid"><div className="footer-brand"><div className="brand inverse"><ToothIcon/><strong>DentalBright</strong></div><p>Clínica dental premium comprometida con tu salud, bienestar y la excelencia en cada sonrisa.</p><div className="socials"><a href="#" aria-label="Facebook"><Facebook/></a><a href="#" aria-label="Instagram"><Instagram/></a><a href="#" aria-label="WhatsApp"><MessageCircle/></a><a href="#" aria-label="YouTube"><Youtube/></a></div></div><div><h3>Enlaces rápidos</h3><a href="#inicio">Inicio</a><a href="#servicios">Servicios</a><a href="#clinica">Sobre Nosotros</a><a href="#contacto">Contacto</a></div><div><h3>Servicios</h3>{services.map(s=><a href="#servicios" key={s.title}>{s.title.replace('\n',' ')}</a>)}</div><div><h3>Información</h3><p><MapPin/> Av. Insurgentes Sur 1234<br/>Col. Del Valle, CDMX</p><p><Phone/> (55) 1234 5678</p><p><Clock3/> Lun–Vie: 9:00 am – 7:00 pm<br/>Sáb: 9:00 am – 2:00 pm</p></div><div><h3>Certificaciones</h3><div className="cert"><span className="seal"><Award/></span><span>Asociación Dental<br/>Mexicana</span></div><div className="cert"><span className="seal second"><ToothIcon/></span><span>Consejo Mexicano de<br/>Odontología</span></div></div></div><div className="copyright"><span>© 2026 DentalBright. Todos los derechos reservados.</span><span>Aviso de Privacidad　 Términos y Condiciones</span></div></footer>
  </main>
}
